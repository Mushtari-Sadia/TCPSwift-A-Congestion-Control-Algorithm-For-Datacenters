...
int main (int argc, char *argv[])
{
  ...
  //params
  uint16_t num_flows = 60; //10,20,30,40,60,80
  uint16_t num_nodes = 40; //20, 40, 60, 80, and 100
...
// Create a new directory to store the output of the program
  std::string dir = "Calculations/task-a/my-"+transport_prot +"-calculations/" + currentTime + "/";
  std::string dirToSave = "mkdir -p " + dir;
  if (system (dirToSave.c_str ()) == -1)
    {
      exit (1);
    }
  std::string dir_summary = "Calculations/task-a/my-"+transport_prot +"-calculations/nodes="+std::to_string(num_nodes)+",";
