...
TcpSocketState::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TcpSocketState")
    ...
    .AddAttribute ("TimeofLastDecrease","Time of last decrease",
                 EmptyAttributeValue (),
                   MakeTimeAccessor (&TcpSocketState::t_last_decrease),
                   MakeTimeChecker ())
    .AddAttribute ("CanDecrease","Set boolean can decrease",
                  BooleanValue (false),
                 MakeBooleanAccessor (&TcpSocketState::m_canDecrease),
                   MakeBooleanChecker ())
    .AddAttribute ("RetransmitCount", "Retransmit Count",
                   UintegerValue (0),
                   MakeUintegerAccessor (&TcpSocketState::retransmit_count),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("max_cwnd", "Maximum Congestion Window",
                   UintegerValue (20000),
                   MakeUintegerAccessor (&TcpSocketState::max_cwnd),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("min_cwnd", "Maximum Congestion Window",
                   UintegerValue (0),
                   MakeUintegerAccessor (&TcpSocketState::min_cwnd),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("Gamma", "Limit on increase",
                   UintegerValue (1),
                   MakeUintegerAccessor (&TcpSocketState::m_gamma),
                   MakeUintegerChecker<uint32_t> ())
