...
TcpSocketBase::ReTxTimeout ()
{
  ...
  //modification
  m_tcb->retransmit_count += 1;
  if (m_tcb->retransmit_count>m_retxThresh)
  {
    m_tcb->m_cWnd = m_tcb->min_cwnd;
    // std::cout << "m_tcb->retransmit_count>m_retxThresh" << std::endl;
  }
  else
  {
    if (m_tcb->m_canDecrease)
    {
      uint32_t segCwnd = m_tcb->GetCwndInSegments ();
      m_tcb->m_cWnd = (1-m_tcb->m_gamma)*m_tcb->m_cWnd*m_tcb->m_segmentSize*segCwnd;
      // std::cout << "m_tcb->m_canDecrease" << std::endl;
    }
  }
